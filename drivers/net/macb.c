...
#define MACB_TX_TIMEOUT		1000000
...
static int macb_phy_find(struct macb_device *macb)
{
	...
	for (i = 0; i < 32; i++) {
		...
		phy_id = macb_mdio_read(macb, MII_PHYSID1);
                 printf("macb_phy_find PHYSID1: %4x \n" , macb_mdio_read(macb, MII_PHYSID1));
                 printf("macb_phy_find PHYSID2: %4x \n" , macb_mdio_read(macb, MII_PHYSID2));
		if (phy_id != 0xffff) {
...
static int macb_phy_init(struct macb_device *macb)
{
	...
	phy_id = macb_mdio_read(macb, MII_PHYSID1);
	printf("macb_phy_init PHYSID1:%x \n" , macb_mdio_read(macb, MII_PHYSID1));
	printf("macb_phy_init PHYSID2:%x \n" , macb_mdio_read(macb, MII_PHYSID2));
	if (phy_id == 0xffff) {
